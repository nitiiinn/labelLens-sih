...
/**
 * Escalate inspection to Controller (Reviewer QA action)
 */
export async function escalateInspection(req, reply) {
  try {
    const { inspectionId } = req.params;
    const inspection = await inspectionService.escalateInspection(
      req.user,
      inspectionId
    );
    ...
    return reply.code(200).send({
      message: "Inspection escalated to Controller",
      inspection,
    });
  } catch (error) {
    if (error instanceof SecurityScopingError) {
    ...
    return reply.code(500).send({
      error: "Internal Server Error",
      message: "Failed to escalate inspection",
    });
  }
}

/**
 * Approve an inspection's AI result (Reviewer QA action)
 */
export async function approveInspection(req, reply) {
  try {
    const { inspectionId } = req.params;
    const inspection = await inspectionService.approveInspection(req.user, inspectionId);
    ...
    return reply.code(200).send({
      message: "Inspection approved by Reviewer",
      inspection,
    });
  } catch (error) {
    if (error instanceof SecurityScopingError) {
    ...
    return reply.code(400).send({
      error: "Bad Request",
      message: error.message || "Failed to approve inspection",
    });
  }
}
