 ...
 *
 * Scoping Matrix:
 * - DIRECTOR & ASSISTANT_DIRECTOR: Global access (no restrictive where clauses).
 ...
 * - MANUFACTURER: Queries must filter by org: where: { manufacturerId: req.user.organizationId }
...
export function getInspectionScope(user) {
  ...
  switch (role) {
    ...
    case ROLES.MANUFACTURER:
      if (!user.organizationId || !String(user.organizationId).trim()) {
        throw new SecurityScopingError(
          "MANUFACTURER account is not linked to an organization. Access denied.",
          { role, userId: user.id }
        );
      }
      return { manufacturerId: String(user.organizationId).trim() };
  ...
  switch (role) {
    ...
    case ROLES.MANUFACTURER:
      if (!user.organizationId) {
        return { id: "__UNAUTHORIZED_REPORT_ACCESS__" };
      }
      return {
        inspection: {
...
export function canAccessInspection(user, inspection) {
  ...
  switch (role) {
    ...
    case ROLES.MANUFACTURER:
      return (
        !!user.organizationId &&
        inspection.manufacturerId === user.organizationId
      );
