...
/**
 * Escalates an inspection to the Controller (Reviewer QA action).
 * Flags the record with status 'ESCALATED' so it surfaces in the
 * Controller's escalation hub.
 *
 * @param {import("../constants/rbac.js").AuthenticatedUserContext} user
 * @param {string} inspectionId
 */
export async function escalateInspection(user, inspectionId) {
  const inspection = await getScopedInspectionById(user, inspectionId);
  if (!inspection) {
    throw new Error("Inspection not found or inaccessible");
  }
  ...
  return prisma.inspection.update({
    where: { id: inspectionId },
    data: { status: "ESCALATED", reviewerId: user.id },
    select: { id: true, status: true, reviewerId: true },
  });
}

/**
 * Approves an inspection's AI result for the current reviewer's district.
 * The compliance result is preserved; reviewerId marks the QA decision.
 */
