...
async function migrate() {
  ...
  try {
    ...
    await client.query(`
      DO $$ BEGIN
        CREATE TYPE "UserRole" AS ENUM ('DIRECTOR', 'ASSISTANT_DIRECTOR', 'CONTROLLER', 'REVIEWER', 'INSPECTOR', 'MANUFACTURER', 'CONSUMER');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;
    `);

    for (const val of ["DIRECTOR", "ASSISTANT_DIRECTOR", "CONTROLLER", "REVIEWER", "INSPECTOR", "MANUFACTURER", "CONSUMER"]) {
      try {
    ...
    await client.query(`
      ...
      UPDATE "users" SET "role" = 'ASSISTANT_DIRECTOR' WHERE "role"::text IN ('STATE_CONTROLLER');
      UPDATE "users" SET "role" = 'DIRECTOR' WHERE "role"::text IN ('ADMIN', 'admin');
      UPDATE "users" SET "role" = 'CONSUMER' WHERE "role"::text NOT IN ('DIRECTOR', 'ASSISTANT_DIRECTOR', 'CONTROLLER', 'REVIEWER', 'INSPECTOR', 'MANUFACTURER', 'CONSUMER');
    `);
